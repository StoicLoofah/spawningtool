...
class GameParser(object):
    ...
    def get_parsed_data(self, cutoff_time=None, cache_dir=None, include_map_details=False):
        ...
        self.parsed_data = {
            ...
            'cooperative': self.replay.cooperative or any(player.commander for player in self.replay.players),
        }
    ...
    def parse_events(self, cutoff_time, include_map_details=False):
        ...
        self.set_constants()
        self.parsed_data['frames_per_second'] = self.constants.FRAMES_PER_SECOND
    ...
    def set_constants(self):
        ...
        if self.parsed_data['cooperative']:
            ...
            # do not incorporate LotV logic below
            return
        elif self.replay.expansion == 'LotV':
    ...
    def set_chronoboost_data(self):
        ...
        elif self.replay.expansion == 'LotV' and 1510617600 > self.replay.unix_timestamp > 1441238400 or \
        ...
        self.chronoboost_multiplier = CHRONOBOOST_MULTIPLIER[chronoboost_version]
    ...
    def add_ability_event(self, event):
        ...
        if ability_name not in self.constants.TRACKED_ABILITIES or not player:
            return

        self.abilities[player].add_event(AbilityEvent(ability_name, event.frame,
                                                      self.constants.FRAMES_PER_SECOND))
    ...
    def add_unit_born_event(self, event):
        ...
        if unit_name in self.constants.BO_EXCLUDED or player == 0 or event.unit.hallucinated:
            return

        if not unit_name in self.constants.BUILD_DATA:
            unit_name = event.unit.name
            if unit_name in self.constants.BO_EXCLUDED:
                return
        ...
        if unit_name is None:
            ...
            frame, unit_name, is_chronoboosted = self.adjust_build_time(event, player, unit_name)
        ...
        self.builds[player].add_event(BuildEvent(unit_name, frame, self.constants.FRAMES_PER_SECOND, supply,
    ...
    def add_upgrade_event(self, event):
        ...
        if unit_name in self.constants.BO_UPGRADES_EXCLUDED:
            return

        if unit_name in self.constants.BUILD_DATA:
            frame, unit_name, is_chronoboosted = self.adjust_build_time(event, player, unit_name)
        else:
        ...
        self.builds[player].add_event(BuildEvent(
            unit_name, frame, self.constants.FRAMES_PER_SECOND, supply, is_chronoboosted=is_chronoboosted))
    ...
    def add_change_event(self, event):
        ...
        unit_name = event.unit_type_name
        if player == 0 or unit_name in self.constants.BO_CHANGED_EXCLUDED:
            return

        try:
            frame = event.frame - self.constants.BUILD_DATA[unit_name]['build_time']
        except KeyError:
            return
    ...
    def add_died_event(self, event):
        ...
        if unit_name in self.constants.BO_EXCLUDED or player == 0:
            return

        killer = event.killer_pid

        self.units_lost[player].add_event(DiedEvent(unit_name, event.frame, self.constants.FRAMES_PER_SECOND, killer,
            self.get_clock_position(event)))

    def adjust_build_time(self, event, player, unit_name):
        """
        ...
        cur_build_data = self.constants.BUILD_DATA[unit_name]

        projected_start = frame - self.constants.BUILD_DATA[unit_name]['build_time']
        chronoboosted = False
    ...
    def make_event_timeline(self, timelines, cutoff_time, field):
        ...
        for i, timeline in timelines.items():
            ...
            for event in timeline.timeline:
                ...
                current_gametime = _frame_to_time(event.frame, self.constants.FRAMES_PER_SECOND)
                if (cutoff_time and
